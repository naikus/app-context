...
function createAppContext() {
  ...
  const rootContext = {
    register(moduleDefn) {
      ...
      if(started) {
        // Since initialize() can be called from other modules, we ALWAYS want to use the :root: context
        // to do the initialization to provide correct dependency resolution. Using "this" will cause 
        // the modules to use their module-specific contexts to initialize which will result in 
        // incorrect circular dependencies
        initializeModule(rootContext, modInfo);
    ...
    async start() {
      ...
      const modInfos = Array.from(modules.values()),
          ...
          unsub = emitter.on("context:init-module", function checkAllModulesLoaded(/* modInfo */) {
            const mInfos = Array.from(modules.values()),
                initializedMods = mInfos.filter(m => m.initialized);
            if(initializedMods.length === mInfos.length) {
              unsub();
      ...
      for(const modInfo of modInfos) {
        // This is an intentional then() on the same promise to ensure that then handlers
        ...
        initPromise.then(() => initializeModule(rootContext, modInfo)).catch(err => {
          reject(err);
        });
